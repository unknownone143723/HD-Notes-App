...
const InputField = ({ name, type, label, value, onChange, icon, disabled = false, required = true }: any) => ( <div className="relative"> <input id={name} name={name} type={type} value={value} onChange={onChange} className="block w-full px-3 py-3 text-sm text-gray-900 bg-transparent rounded-lg border border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer disabled:opacity-70" placeholder=" " required={required} disabled={disabled} /> <label htmlFor={name} className="absolute text-sm text-gray-500 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-white px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 start-1" > {label} </label> {icon && <div className="absolute top-1/2 -translate-y-1/2 right-3 text-gray-400">{icon}</div>} </div> );
            ...
            <div className="w-full max-w-4xl md:mt-2 md:h-[620px] mx-auto bg-white rounded-2xl flex">
                <div className="w-full lg:w-1/2 p-8 sm:p-12 flex flex-col justify-center">
                    ...
                    <form onSubmit={handleVerifyLoginOtp} className="space-y-6">
                        ...
                        {step === 2 && (
                            <InputField name="otp" type="text" label="OTP" value={otp} 
                                onChange={(e: any) => {
